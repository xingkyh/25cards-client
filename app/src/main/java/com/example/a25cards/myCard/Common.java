	...
	public static void rePosition(MyView view, List<Card> list, int flag) {
		if(flag==1)
		{
			for(int i=0,len=list.size();i<len;i++){
				...
				if(card.clicked)
					y-=card.height/3;
				card.setLocation(view.screen_width/2-(len/2-i)*card.width/3,y);
			}
		}
		if(flag==0)
		{
		...
		if (flag==3)
		{
			for(int i=0,len=list.size();i<len;i++){
				Card card=list.get(i);
				card.setLocation(view.screen_width/2-(len/2-i)*card.width/3,0);
			}
		}
	}
	// 返回值
	...
	public static int checkCards(List<Card> c, List<Card> oppo) {
		...
		if (cType == CardType.c4) {
			...
			if(cType2!=CardType.c4)
			{
			 ...
			 if(c.size() != oppo.size() && oppo.size() != 2)
			 {
			 	if (c.size() > oppo.size())
				{
					return 1;
